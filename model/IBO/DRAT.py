...
class DA(nn.Module):  # Residual Degradation-driven Attention (RDA)
    ...
    def forward(self, lr, ldp):
        """
        Input: lr: (B, H*W, C), ldp: (B, 256)
        Output: lr: (B, H*W, C)
        """
        B, N, C = lr.shape
        ldp = self.kernel(ldp).view(-1, 1, C)
        shortcut = lr * ldp
        lr = shortcut + lr
        return lr


class RA(nn.Module):  # Residual Reconstruction-guided Attention (RRA)
    # The implementation builds on CDFormer https://github.com/I2-Multimedia-Lab/CDFormer
    def __init__(self, dim):
        super().__init__()
    ...
    def forward(self, lr, sdp):
        """
        Input: lr: (B, H*W, C), sdp: (B, 256)
        Output: lr: (B, H*W, C)
        """
        B, N, C = lr.shape
        sdp = self.kernel(sdp).view(-1, 1, C * 2)
        sdp1, sdp2 = sdp.chunk(2, dim=2)
        shortcut = lr * sdp1 + sdp2
        lr = shortcut + lr
        return lr


class DRAB(nn.Module):  # Degradation-driven and Reconstruction-guided Attention Block
    ...
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
        ...
        self.dra1 = DA(dim) if IBO == 'upper' else RA(dim)
        self.dra2 = DA(dim) if IBO == 'upper' else RA(dim)

    def forward(self, x, dp, x_size):
        """
        Input: x: (B, H*W, C), x_size: (H, W), dp: (B, 256)
        Output: x: (B, H*W, C)
        """
        H, W = x_size
        x = x + self.drop_path(self.attn(self.dra1(self.norm1(x), dp), H, W))
        x = x + self.drop_path(self.ffn(self.dra2(self.norm2(x), dp), H, W))
        return x
...
if __name__ == '__main__':
    ...
    model_upper = DRAT(IBO='upper')
    model_lower = DRAT(IBO='lower')
    dp = torch.randn((batch_size, 256))
    # print(model)
    # model_upper.eval()
    # model_lower.eval()
    ...
    y1 = model_upper(x, dp)
    y2 = model_lower(x, dp)
    print('dat shape', y1.shape)
    print('rat shape', y2.shape)
